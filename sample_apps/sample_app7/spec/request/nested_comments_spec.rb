...
module SampleApp7
  RSpec.describe 'Nested comments and persistence boundaries' do
    before do
      ...
      Card.dataset.delete
      @parent = Card.create(title: 'Owner')
      @other = Card.create(title: 'Other')
      @comment = Comment.create(card_id: @parent.id, content: 'Original')
    end

    def nested(parent = @parent)
      "/cards/#{parent.id}/comments/#{@comment.id}"
    end
    ...
    it 'uses the URL parent even when query and body try to supply another parent' do
      post "/cards/#{@parent.id}/comments?card_id=#{@other.id}",
           JSON.generate(card_id: @other.id, content: 'Safe'), 'CONTENT_TYPE' => 'application/json'
      expect(last_response.status).to eq(201)
      expect(resp[:comment][:card_id]).to eq(@parent.id)
    end
    ...
    it 'saves a new comment once and returns 422 without inserting invalid data' do
      ...
      post "/cards/#{@parent.id}/comments", content: 'Once'
      expect(last_response.status).to eq(201)
      expect(calls).to eq(1)
      expect { post "/cards/#{@parent.id}/comments", author: 'No content' }.not_to change(Comment, :count)
    ...
    it 'runs parent destroy hooks and enforces cascading foreign keys' do
      ...
      delete "/cards/#{@parent.id}"
      expect(last_response.status).to eq(200)
      expect(Comment[@comment.id]).to be_nil
      expect { Comment.create(card_id: @parent.id, content: 'Orphan') }.to raise_error(Sequel::ForeignKeyConstraintViolation)
    end

    it 'updates timestamps and limits nested collections' do
      old = Time.now - 3600
      @comment.update(updated_at: old)
      Comment.create(card_id: @parent.id, content: 'Second')
      get "/cards/#{@parent.id}/comments", limit: 1
