# Todo API with custom responses

A runnable MK Framework sample using the shared library at `../../lib`.
Its Ruby namespace is `SampleApp3` and its Rack entrypoint is `SampleApp3::App.app`.
...
The index response wraps records in `todos` alongside `custom_field`. Create
and delete responses also demonstrate application-specific envelope fields.
